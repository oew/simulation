...
public class MapSimulation {
	...
	private static final Logger logger = LogManager.getLogger("MapSimulation");
	...
	public void run() {
		...
		logger.info("Load time " + (System.currentTimeMillis() - now));
	...
	public List<MapFactory> getMaploaders() {
		if (maploaders == null) maploaders = new LinkedList<MapFactory>();
		return maploaders;
	}

	public List<TrendExecutor> getTrendExecutors() {
		if (trendExecutors == null) trendExecutors = new LinkedList<TrendExecutor>();
		return trendExecutors;
	}
	
	public String toString() {
		return "MapSimulation [maploaders="+maploaders.size() + ", TrendsExecutors=" + trendExecutors.size() + "Refresh Rates =  "+ refreshMillis +"]";
	}
